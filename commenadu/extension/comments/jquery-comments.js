...
(function (factory) {
    ...
    var Comments = {
        ...
        postComment: function(ev) {
            ...
            ///// Custom code - Ryan
            // chrome.runtime.sendMessage({type:"success", message: commentJSON.highlightID}, (response) => {
        ...
        deleteComment: function(ev) {
            ...
            /** Custom code - Ryan
             * Only remove formatting of the highlighted text on the page if all the comments about it are removed.
             * If only one comment is referring to a specific highlight, then the highlight will be deleted.
             */
            if(commentJSON.highlightID)
            {
                var commentsAboutSameHighlight = $('li[id=' + commentJSON.highlightID + ']');
                if(commentsAboutSameHighlight.length <= 1)
                {
                    chrome.tabs.query({active:true, currentWindow:true}, function(tabs){
                        chrome.tabs.sendMessage(tabs[0].id, {type:"delete", message: commentJSON.highlightID}, (response) => {
                            console.log(response);
                        });
                    });
                }
